# AWS JENKINS INFRA

This tutorial I will create the jenkins control node on EC2 ubuntu and with ALB so that i can access it from internet.

## Features

- Jenkins control node site running on docker container
- docker running on EC2 ubuntu node
- ALB for web access 

## Usage

- Edit the terraform.tfvars file to provide your own aws information:

```hcl
aws_region = "us-east-1"
vpc_id     = "vpc-0a346dceeb8b922a6"
public_subnet_ids = [
  "subnet-0102416d2c47883c4",
  "subnet-0e3c240984542b792",
  "subnet-09f49cec00c38bdc1",
] #this is for alb to use
private_subnet_id = "subnet-0c33850d4d6866396"
instance_type     = "t3.micro"
ubuntu_version    = "20.04"
key_pair_name     = "ssh-for-jenkins-server"
# this is the id_rsa.pub content of your bastion server
public_key_content = "ssh-rsa  ubuntu@ip-172-20-1-47"

```
## Notie

- you can use the shell to get jenkins initial password 
```shell
docker exec jenkins cat /var/jenkins_home/secrets/initialAdminPassword

```






